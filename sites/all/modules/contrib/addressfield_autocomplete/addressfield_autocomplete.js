    ...
    if (!o.hasClass('manual-add') || o.hasClass('reverse-geocode')) {
      ...
      $.each(widget.find('[data-geo]'), function() {
        ...
        for (var i = 0; i < geo.length; i++) {
          ...
          if (!$(this).is('select') || $('option[value="' + data[geo[i]] + '"]', this).length) {
            $(this).val(data[geo[i]]);
            continue;
          }

          // Google doesn't always return short names
          if($('option:contains(' + data[geo[i]] + ')', this).length) {
            $(this).val($('option:contains(' + data[geo[i]] + ')', this).val());
          }
        }
      });
    }
